...
import { myHasPermission } from '@/utils/permission'
...
export function filterRoutes(routes, rolesControl, baseUrl = '/') {
  return routes
    .filter((route) =>
      rolesControl && route.meta && route.meta.permissionId
        ? myHasPermission(route.meta.permissionId)
        : true
    )
    .map((route) => {
      route = { ...route }
