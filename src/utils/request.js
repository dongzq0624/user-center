...
const requestConf = (config) => {
  const token = store.getters['user/token'],
    clientToken = store.getters['user/clientToken']

  // 不规范写法 可根据setting.config.js tokenName配置随意自定义headers
  // if (token) config.headers[tokenName] = token
  // 规范写法 不可随意自定义
  if (token) config.headers['xc_sso_sessionid'] = `${token}`
  if (clientToken) config.headers['xc_client_sessionid'] = clientToken
  if (
    config.data &&
 ...
 * @returns {any} 返回结果
 */
const tryRefreshClientToken = async (config) => {
  if (!refreshClientToken) {
    ...
    try {
      ...
      if (clientToken) {
        store.dispatch('user/setClientToken', clientToken).then(() => {})
        // 已经刷新了token，将所有队列中的请求进行重试
        requests.forEach((cb) => cb(clientToken))
        requests = []
        return instance(requestConf(config))
      }
    } catch (error) {
...
const handleData = async ({ config, data, status, statusText }) => {
  ...
  switch (code) {
    case 200:
      ...
      return data
    case 80000001:
      return await tryRefreshClientToken(config)
    case 99995000:
      return await tryRefreshClientToken(config)
    case 401:
      store
...
instance.interceptors.response.use(
  ...
  (error) => {
    ...
    if (response === undefined) {
      if (loadingInstance) loadingInstance.close()
      Vue.prototype.$baseMessage('服务器异常', 'error', 'vab-hey-message-error')
      return {}
