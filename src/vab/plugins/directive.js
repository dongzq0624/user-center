import Vue from 'vue'
import { myHasPermission, projectPermission } from '@/utils/permission'

/**
 * @description 自定义指令v-permissions  系统权限
 */
Vue.directive('permissions', {
  inserted(el, binding) {
    const { value } = binding
    if (value) {
      if (!myHasPermission(value)) {
...
/**
 * @description 自定义指令v-projectPermissions  项目权限
 */
Vue.directive('projectPermissions', {
  inserted(el, binding) {
    const { value } = binding
    if (value) {
      if (!projectPermission(value)) {
        el.parentNode && el.parentNode.removeChild(el)
      }
    }
  },
